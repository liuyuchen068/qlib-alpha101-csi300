...
class HS300DataPreparator:
    ...
    def get_previous_close(self,df_prev:pd.DataFrame) ->dict[str,float]:
        """获取前一日的收盘价"""
        prev_close_dict={}
        if df_prev is not None and not df_prev.empty:
            for idx, row in df_prev.iterrows():
                ...
                if pd.notna(close_price) and close_price>0:
                    prev_close_dict[code] = close_price
    ...
    def get_stock_type(self,code:str) -> str:
        ...
        if code.endswith(".XSHG"):
            ...
            code_num=code.split(".")[0]
            if code_num.startswith("00") or code_num.startswith("001") or code_num.startswith("002") or code_num.startswith("003"):
                return "深圳主板"
            elif code_num.startswith("300"):
                return "创业板"
            elif code_num.startswith("83") or code_num.startswith("87"):
                return "新三板"
            else:
                return "深圳主板"
        else:
    ...
    def get_limit_threshold(self,code:str,is_st:int) ->Optional[float]:
        """获取股票的涨停跌停阈值"""
        if is_st==1:
            return 0.05
        stock_type=self.get_stock_type(code)
        if stock_type=="上海主板" or stock_type=="深圳主板":
            return 0.10
        elif stock_type=="创业板":
            return 0.20
        elif stock_type=="科创板":
    ...
    def add_risk_flags(self,df_price:pd.DataFrame,prev_close_dict) -> pd.DataFrame:
        """添加风险标志列"""
        df_price['is_suspended']=(df_price['volume'] == 0).astype(int)
        df_price['prev_close']=df_price['code'].map(prev_close_dict)
        df_price['prev_close'] = df_price['prev_close'].replace(0, np.nan)
        df_price['is_limit_up']=0
        df_price['is_limit_down']=0
        valid=(df_price['prev_close'] >0) & (df_price['volume']>0)
        df_price.loc[valid, 'daily_return'] = (
            (df_price.loc[valid, 'close'] - df_price.loc[valid, 'prev_close']) / 
            df_price.loc[valid, 'prev_close']
        )
        for idx in df_price[valid].index:
            code=df_price.loc[idx,'code']
            daily_return=df_price.loc[idx,'daily_return']
            is_st=int(df_price.loc[idx,'is_st'])
            limit_threshold=self.get_limit_threshold(code,is_st)
            if limit_threshold is None:
                df_price.at[idx,'is_limit_up']=0
                df_price.at[idx,'is_limit_down']=0
            else:
                if daily_return>limit_threshold:
                    df_price.at[idx,'is_limit_up']=1
                elif daily_return<-limit_threshold:
                    df_price.at[idx,'is_limit_down']=1
                else:
                    df_price.at[idx,'is_limit_up']=0
                    df_price.at[idx,'is_limit_down']=0    
        df_price=df_price.drop(columns=['prev_close','daily_return'],errors='ignore')
        return df_price
    ...
    def fetch_daily_data(self, date_str: str,prev_close_dict:Optional[dict]=None) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            #获取收盘价格
            prev_close_dict = self.get_previous_close(df_price[['code', 'close']])
            #处理缺失值
            for code in stocks:
    ...
    def collect_all_data(self) -> pd.DataFrame:
        ...
        # 保证列顺序
        base_cols = ["code", "date", "open", "close", "high", "low", "volume", "factor","is_suspended", "is_limit_up", "is_limit_down","is_st"]
...
def main():
    try:
        preparator = HS300DataPreparator()  #创建实例
        preparator.run()
    except Exception as e:
