...
- **策略回测**：构建权重策略并进行历史回测
- **风险控制**：通过akshare识别并排除停牌、ST、涨跌停等风险股票
...
│── data/              # 数据准备层
│   ├── __init__.py
│   ├── preparator.py  # HS300DataPreparator（主类）
│   ├── risk.py        # 风控字段逻辑（涨停/ST等）
│   └── utils.py       # 日期/股票分类等工具函数
│
│── dataset/           # 数据集构建（qlib）
│   ├── builder.py     # DataBuilder
│   └── loader.py      # QlibDataLoader封装（可选）
│
│── model/             # 模型层
│   └── lgb_model.py   # ModelManager
│
│── strategy/          # 策略层
│   └── score_strategy.py  # ScoreWeightedStrategy
│
│── backtest/          # 回测执行
...
- **akshare 账号**：需提前注册 [https://akshare.pro/](https://akshare.pro/)

## 🚀 快速开始

### 1.运行代码：

如果选用方案1，需要先运行数据准备脚本，再运行回测脚本：
...
如果选用方案2，需要先运行数据准备脚本，再运行 qrun 脚本：

```bash
python -m scripts.run_data
python qrun/run_qrun.py
```
### 2. 安装依赖
```bash
...
确保 `config.yaml` 或 `qrun/config_qrun.yaml` 中的数据路径与实际目录一致。
...
- 通过akshare识别 ST 股、停牌股、涨跌停情况
- 输出：`data/input/my_data/SH??????.csv`、`SZ??????.csv`

### 第二步：数据格式转换
将 CSV 数据转换为 Qlib 需要的 bin 格式：
```bash
python dump_bin.py
```
...
- **akshare 文档**：[https://akshare.pro/](https://akshare.pro/)
