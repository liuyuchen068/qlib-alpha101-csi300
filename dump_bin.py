"""
修复版本：自动处理 NaTType（缺失日期值）错误

关键改动：
1. _format_datetime - 添加 NaT 检查
2. _get_source_data - 删除有问题的日期
3. _get_all_date - 过滤 None 值
"""
...
class DumpDataBase:
    ...
    def _format_datetime(self, datetime_d: [str, pd.Timestamp]):
        ...
        ✅ 改动：添加 NaT 检查
        如果日期是 NaT（缺失值），返回 None
        """
        datetime_d = pd.Timestamp(datetime_d)
        
        # ✅ 检查是否为 NaT（Not a Time）
        if pd.isna(datetime_d):
            logger.warning(f"遇到缺失的日期值（NaT），已跳过")
            return None
    ...
    def _get_source_data(self, file_path: Path) -> pd.DataFrame:
        ...
        if self.date_field_name in df.columns:
            ...
            if before_count > after_count:
                logger.warning(
                    f"文件 {file_path.name}: 删除了 {before_count - after_count} "
                    f"行有缺失日期的数据 (保留 {after_count} 行)"
                )
    ...
    def save_calendars(self, calendars_data: list):
        ...
        # ✅ 过滤掉 None 值（来自 NaT）
        result_calendars_list = [x for x in result_calendars_list if x is not None]
...
class DumpDataAll(DumpDataBase):
    def _get_all_date(self):
            ...
            with ProcessPoolExecutor(max_workers=self.works) as executor:
                for file_path, ((_begin_time, _end_time), _set_calendars) in zip(
                    ...
                    if isinstance(_begin_time, pd.Timestamp) and isinstance(_end_time, pd.Timestamp):
                        ...
                        # ✅ 检查格式化是否成功（可能返回 None）
                        if _begin_time is not None and _end_time is not None:
        ...
        # ✅ 过滤掉 NaT 值
        all_datetime = set(filter(lambda x: pd.notna(x), all_datetime))
...
if __name__ == "__main__":
    data_path = r"C:\Users\ASUS\Desktop\my-project\my_data"
    qlib_dir = r"C:\Users\ASUS\Desktop\my-project\qlib_data"
    include_fields = "open,close,high,low,volume,factor,alpha_001,alpha_002,alpha_003,alpha_004,alpha_005,alpha_006,alpha_007,alpha_008,alpha_009,alpha_010,is_suspended,is_limit_up,is_limit_down,is_st"
    d = DumpDataAll(
        data_path=data_path,
        qlib_dir=qlib_dir,
